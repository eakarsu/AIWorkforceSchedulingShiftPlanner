...
export default function TimeOff() {
  ...
  const typeClass = (t) => ({ vacation: 'blue', sick: 'red', personal: 'purple', family: 'yellow' }[t] || 'gray');

  const blank = { employee_id: '', start_date: '', end_date: '', type: 'vacation', reason: '' };
  const openCreate = () => { setForm(blank); setEditItem(null); setShowForm(true); };
  const openEdit = (item) => {
    setForm({ employee_id: item.employee_id || '', start_date: item.start_date ? item.start_date.split('T')[0] : '', end_date: item.end_date ? item.end_date.split('T')[0] : '', type: item.type || 'vacation', reason: item.reason || '' });
  ...
  const handleSave = async (e) => {
    ...
    try {
      const url = editItem ? `/api/time-off/${editItem.id}` : '/api/time-off';
  ...
  const handleAction = async (id, status) => {
    try { await apiFetch(`/api/time-off/${id}`, { method: 'PUT', body: JSON.stringify({ status }) }); setSelected(null); load(); }
    catch { alert('Failed to update'); }
  };

  const handleDelete = async (id) => {
    try { await apiFetch(`/api/time-off/${id}`, { method: 'DELETE' }); setShowDelete(null); setSelected(null); load(); }
    catch { alert('Failed to delete'); }
  };

  return (
    <>
      <div className="page-header"><h2>Time Off Requests</h2><button className="btn btn-primary" onClick={openCreate}><Plus size={16} /> Add New</button></div>
      <div className="page-body">
        <div className="card">
          {loading ? <div className="loading-container"><div className="spinner"></div> Loading...</div> : (
            <div className="table-wrapper">
              <table>
                <thead><tr><th>Employee</th><th>Start Date</th><th>End Date</th><th>Type</th><th>Status</th><th>Reason</th></tr></thead>
                <tbody>
                  {items.length === 0 ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: 'var(--text-secondary)' }}>No time off requests</td></tr> :
                    items.map(item => (
                      <tr key={item.id} onClick={() => setSelected(item)}>
                        <td style={{ fontWeight: 600 }}>{empName(item.employee_id)}</td>
                        <td>{fmtDate(item.start_date)}</td><td>{fmtDate(item.end_date)}</td>
      ...
      {selected && (
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h3>Time Off Details</h3><button className="modal-close" onClick={() => setSelected(null)}><X size={16} /></button></div>
            <div className="modal-body">
              {[['Employee', empName(selected.employee_id)], ['Start Date', fmtDate(selected.start_date)], ['End Date', fmtDate(selected.end_date)],
                ['Type', selected.type], ['Status', selected.status], ['Reason', selected.reason || '-']
              ].map(([l, v]) => (
                <div key={l} className="detail-row"><div className="detail-label">{l}</div>
                  <div className="detail-value">{l === 'Status' ? <span className={`badge ${statusClass(v)}`}>{v}</span> : l === 'Type' ? <span className={`badge ${typeClass(v)}`}>{v}</span> : v}</div></div>
              ))}
            </div>
      ...
      {showForm && (
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h3>{editItem ? 'Edit Time Off' : 'Request Time Off'}</h3><button className="modal-close" onClick={() => setShowForm(false)}><X size={16} /></button></div>
              ...
              <div className="modal-body">
                ...
                <div className="form-group"><label>Type</label>
                  <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })}>
                    <option value="vacation">Vacation</option><option value="sick">Sick</option><option value="personal">Personal</option><option value="family">Family</option>
                  </select></div>
      ...
      {showDelete && (
        <div className="modal-overlay" onClick={() => setShowDelete(null)}>
          <div className="modal" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body"><p className="confirm-text">Delete this time off request?</p></div>
            <div className="modal-footer">
